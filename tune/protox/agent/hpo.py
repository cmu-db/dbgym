...
from misc.utils import conv_inputpath_to_abspath, open_and_save
...
def get_free_port(signal_dpath):
    assert (
        signal_dpath.exists()
    ), f"get_free_port() called with a non-existent signal_dpath ({signal_dpath})"
    assert (
        signal_dpath.is_dir()
    ), f"get_free_port() called with a non-directory signal_dpath ({signal_dpath})"
    assert (
        not signal_dpath.is_symlink()
    ), f"get_free_port() called with a symlink signal_dpath ({signal_dpath})"
    MIN_PORT = 5434
    MAX_PORT = 5500
...
def _mutate_common_config(dbgym_cfg, logdir, hpo_config, protox_args):
    ...
    benchmark_config["protox"]["per_query_knob_gen"] = hpo_config[
        "protox_per_query_knob_gen"
    ]
    benchmark_config["protox"]["per_query_scan_method"] = hpo_config[
        "protox_per_query_scan_method"
    ]
    benchmark_config["protox"]["per_query_select_parallel"] = hpo_config[
        "protox_per_query_select_parallel"
    ]
    benchmark_config["protox"]["index_space_aux_type"] = hpo_config[
        "protox_index_space_aux_type"
    ]
    benchmark_config["protox"]["index_space_aux_include"] = hpo_config[
        "protox_index_space_aux_include"
    ]
    ...
    protox_config["protox"]["benchbase_path"] = str(
        os.path.expanduser(protox_config["protox"]["benchbase_path"])
    )
    ...
    if "oltpr_sf" in protox_args:
        ...
        if conf_etree.getroot().find("terminals") is not None:
            conf_etree.getroot().find("terminals").text = str(
                protox_args.oltp_num_terminals
            )
        if conf_etree.getroot().find("works") is not None:
            works = conf_etree.getroot().find("works").find("work")
            if works.find("time") is not None:
                conf_etree.getroot().find("works").find("work").find("time").text = str(
                    protox_args.oltp_duration
                )
            if works.find("warmup") is not None:
                conf_etree.getroot().find("works").find("work").find("warmup").text = (
                    str(protox_args.oltp_warmup)
                )
    conf_etree.write("benchmark.xml")
    # protox_config will be dumped to a .yaml file, so all Path objects have to be converted to strings
    protox_config["protox"]["benchbase_config_path"] = str(
    ...
    # protox_config will be dumped to a .yaml file, so all Path objects have to be converted to strings
    protox_config["protox"]["pgdata_snapshot_path"] = str(
    ...
    protox_config["protox"][
        "default_quantization_factor"
    ] = hpo_config.default_quantization_factor
    ...
    ] = hpo_config.workload_timeout_penalty
    ...
    if "index_vae" in hpo_config:
        # Enable index_vae.
        protox_config["protox"]["index_vae_metadata"][
            "index_vae"
        ] = hpo_config.index_vae

    if "lsc_enabled" in hpo_config:
        protox_config["protox"]["lsc_parameters"][
            "lsc_enabled"
        ] = hpo_config.lsc_enabled
        protox_config["protox"]["lsc_parameters"]["lsc_embed"] = hpo_config.lsc_embed
        protox_config["protox"]["lsc_parameters"][
            "lsc_shift_initial"
        ] = hpo_config.lsc_shift_initial
        protox_config["protox"]["lsc_parameters"][
            "lsc_shift_increment"
        ] = hpo_config.lsc_shift_increment
        protox_config["protox"]["lsc_parameters"][
            "lsc_shift_max"
        ] = hpo_config.lsc_shift_max
        protox_config["protox"]["lsc_parameters"][
            "lsc_shift_after"
        ] = hpo_config.lsc_shift_after
        protox_config["protox"]["lsc_parameters"][
            "lsc_shift_schedule_eps_freq"
        ] = hpo_config.lsc_shift_schedule_eps_freq
...
def _construct_common_config(args):
    ...
    return {
        ...
        "start_reset": tune.sample_from(
            lambda spc: (
                bool(np.random.choice([False, True]))
                if spc["config"]["maximize_state"]
                else False
            )
        ),
        # Discount.
...
def mutate_wolp_config(dbgym_cfg, logdir, hpo_config, protox_args):
    benchmark_config, pg_path, port = _mutate_common_config(
        dbgym_cfg, logdir, hpo_config, protox_args
    )
