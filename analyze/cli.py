...
def analyze_tfevents(tfevents_path: Path) -> None:
    ...
    logging.getLogger(DBGYM_OUTPUT_LOGGER_NAME).info(f"seconds spent resetting: {get_total_instr_time_event(minimal_json, r'.*PostgresEnv_reset$')}")
    logging.getLogger(DBGYM_OUTPUT_LOGGER_NAME).info(f"seconds spent reconfiguring: {get_total_instr_time_event(minimal_json, r'.*PostgresEnv_shift_state$')}")
    logging.getLogger(DBGYM_OUTPUT_LOGGER_NAME).info(f"seconds spent evaluating workload: {get_total_instr_time_event(minimal_json, r'.*Workload_execute$')}")
    logging.getLogger(DBGYM_OUTPUT_LOGGER_NAME).info(f"seconds spent training agent: {get_total_instr_time_event(minimal_json, r'.*(WolpPolicy_train_actor|WolpPolicy_train_critic)$')}")


# The "minimal json" unwraps each summary so that we're left only with the parts that differ between summaries
def tboard_to_minimal_json(tfevent_fpath: str) -> list[dict]:
    minimal_json = []
...
def get_total_instr_time_event(minimal_json, event_regex):
...
# Gets a set of all tags
def get_all_tags(minimal_json):
    all_tags = set()

    for json_summary in minimal_json:
        all_tags.add(json_summary["tag"])
